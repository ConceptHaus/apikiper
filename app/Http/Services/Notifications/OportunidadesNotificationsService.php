...
class OportunidadesNotificationsService
{
        ...
        if (count($oportunidades) > 0) {
                ...
                foreach ($notifications as $key => $notification) {
                    ...
                    foreach ($oportunidades as $index => $oportunidad) {
                        ...
                        if($oportunidad['id_oportunidad'] == $notification['source_id']){
                            // echo " @ if foreach oportunidades -> oportunidad = notification -> ". $notification['source_id'] . "     |     ";
                            ...
                            if ($inactivity_period <= 0) {
                                ...
                                UtilService::createCustomLog("sendNotifications_log", "| line 67 | changeStatusforExisitingOportunidadNotification for notification -> " . $notification['source_id'] . " resuelto");
                            }
                        }
                    }
                }
            ...
            foreach ($oportunidades as $key => $oportunidad) {
                ...
                if (isset($existing_notification->id)) {
                    ...
                    if ($new_inactivity_period > ($max_time_inactivity * ($existing_notification->attempts + 1))) {
                        ...
                        UtilService::createCustomLog("sendNotifications_log", "| line 96 | updateAttemptsAndInactivityforExisitingOportunidadNotification for oportunidad -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['inactivity_period']);
                        SendNotificationService::sendInactiveOportunityNotification($oportunidad);
                        UtilService::createCustomLog("sendNotifications_log", "| line 98 | sendInactiveOportunityNotification for oportunidad colaborador -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                        if ($send_emails == "all") {
                            if (count($admins) > 0) {
                    ...
                    SendNotificationService::sendInactiveOportunityNotification($oportunidad);
                    UtilService::createCustomLog("sendNotifications_log", "| line 119 | sendInactiveOportunityNotification for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                    OportunidadesNotificationsRep::createOportunidadNotification($oportunidad);
                    UtilService::createCustomLog("sendNotifications_log", "| line 121 | createOportunidadNotification for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                    ...
                    if ($send_emails == "all") {
                        if (count($admins) > 0) {
                            ...
                            foreach ($admins as $key_2 => $admin) {
                                ...
                                $oportunidad_for_admin['colaborador_id']    = $admin['id'];
                                SendNotificationService::sendInactiveOportunityNotification($oportunidad_for_admin);
                                UtilService::createCustomLog("sendNotifications_log", "| line 130 | sendInactiveOportunityNotification for oportunidad admin -> " . $oportunidad_for_admin['id_oportunidad'] . " -> " . $oportunidad_for_admin['colaborador_id']);
                                OportunidadesNotificationsRep::createOportunidadNotification($oportunidad_for_admin, true);
                                UtilService::createCustomLog("sendNotifications_log", "| line 132 | createOportunidadNotification for oportunidad admin -> " . $oportunidad_for_admin['id_oportunidad_for_admin'] . " -> " . $oportunidad['colaborador_id']);
                            }
                        }
                    }
                }
                ...
                if (empty($user_settings) or  (isset($user_settings->configuraciones->disable_email_notification_oportunidades) and $user_settings->configuraciones->disable_email_notification_oportunidades == 0)) {
                    if ($oportunidad['inactivity_period'] > 0 and $oportunidad['inactivity_period'] >= ($max_time_inactivity * $oportunidad['attempts']) and ($existing_notification_attempts != $oportunidad['attempts'])) {
                        ...
                        UtilService::createCustomLog("sendNotifications_log", "| line 144 | sendOportunidadNotificationEmail for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                    }
                }
                    ...
                    if (count($admins) > 0) {
                        ...
                        foreach ($admins as $key_2 => $admin) {
                            ...
                            if ($oportunidad_for_admin['inactivity_period'] > 0 and $oportunidad_for_admin['inactivity_period'] >= ($max_time_inactivity * $oportunidad_for_admin['attempts']) and ($existing_notification_attempts != $oportunidad_for_admin['attempts'])) {
                                ...
                                UtilService::createCustomLog("sendNotifications_log", "| line 156 | sendOportunidadNotificationEmail for oportunidad admin -> " . $oportunidad_for_admin['id_oportunidad'] . " -> " . $oportunidad_for_admin['colaborador_id']);
                            }
                        }
                    }
                }
            ...
            if (count($notifications) > 0) {
                foreach ($notifications as $key => $notification) {
                    ...
                    if ($inactivity_period <= 0) {
                        OportunidadesNotificationsService::changeStatusforExisitingOportunidadNotification($notification['source_id'], 'resuelto');
                        UtilService::createCustomLog("sendNotifications_log", "| line 169 | changeStatusforExisitingOportunidadNotification for oportunidad  -> " . $notification['source_id'] . " -> resuelto");
                    }
                }
            }
        }
        ...
        if (count($notifications) > 0) {
            ...
            if (count($admins) > 0) {
                ...
                foreach ($notifications as $key => $notification) {
                    ...
                    UtilService::createCustomLog("sendNotifications_log", "| line 207 | changeStatusforExisitingOportunidadNotification for notification -> " . $notification['source_id'] . " escalado");
                    OportunidadesNotificationsService::sendOportunidadEscalationEmail($notification, $admins);
                    UtilService::createCustomLog("sendNotifications_log", "| line 209 | sendOportunidadEscalationEmail for oportunidad admins -> " . $notification['source_id']);
                    // GET OPORTUNIDAD
                    OportunidadesNotificationsService::sendInactiveOportunityNotificationToAdmins($oportunidad);
                }
                UtilService::createCustomLog("sendNotifications_log", "<!-- escalateNotifications -->");
            }
        }
    ...
    public static function sendInactiveOportunityNotificationToAdmins($oportunidad)
    {
        foreach ($admins as $key => $admin) {
            SendNotificationService::sendInactiveOportunityNotification($oportunidad_for_admin);
            UtilService::createCustomLog("sendNotifications_log", "| line 106 | sendInactiveOportunityNotification for oportunidad admin -> " . $oportunidad_for_admin['id_oportunidad'] . " -> " . $oportunidad_for_admin['colaborador_id']);
                        ...
                        foreach ($oportunidades as $key => $oportunidad) {
                            ...
                            if (isset($existing_notification->id)) {
                                ...
                                if( $new_inactivity_period > $existing_notification->inactivity_period ){
                                    SendNotificationService::sendInactiveOportunityNotification($oportunidad);
                                    UtilService::createCustomLog("sendNotifications_log", "| line 270 | sendInactiveOportunityNotification for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                                    ...
                                    UtilService::createCustomLog("sendNotifications_log", "| line 272 | updateAttemptsAndInactivityforExisitingOportunidadNotification for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                                }
                            } else {
                                ...
                                SendNotificationService::sendInactiveOportunityNotification($oportunidad);
                                UtilService::createCustomLog("sendNotifications_log", "| line 279 | sendInactiveOportunityNotification for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                                OportunidadesNotificationsRep::createOportunidadNotification($oportunidad);
                                UtilService::createCustomLog("sendNotifications_log", "| line 281 | createOportunidadNotification for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                            }
                            ...
                            if (isset($user_settings->configuraciones->disable_email_notification_oportunidades) and !$user_settings->configuraciones->disable_email_notification_oportunidades) {
                                ...
                                if ($inactivity_period > 0 AND $inactivity_period > ($hours * $attempts)) {
                                    ...
                                    UtilService::createCustomLog("sendNotifications_log", "| line 290 | sendOportunidadNotificationColaboradorEmail for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                                }
                                //First notification
                                if ($oportunidad['attempts'] == 0 AND $inactivity_period >= ($hours * $attempts)) {
                                    OportunidadesNotificationsService::sendOportunidadNotificationColaboradorEmail($oportunidad);
                                    UtilService::createCustomLog("sendNotifications_log", "| line 295 | sendOportunidadNotificationColaboradorEmail for oportunidad user -> " . $oportunidad['id_oportunidad'] . " -> " . $oportunidad['colaborador_id']);
                                }
                            }
                        }
                    }
