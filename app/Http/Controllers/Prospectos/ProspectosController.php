   ...
   public function registerProspecto(Request $request){
        ...
        if($validator->passes()){
            try{
                ...
                $prospectoDetalle->anio_2023=$request->anio_2023;
                ...
                $prospectoDetalle->genero=$request->Genero;
                $prospectoDetalle->estatus_socio=$request->estatusSocio;
    ...
    public function updateProspecto(Request $request, $id){
        ...
        try{
            ...
            $detalle->cpf = $request-> CodigoPostalFiscal;
            $detalle->correoof = $request-> CorreoFiscal;
            $detalle->nombrec = $request-> nombreComercial;
            $detalle->correocont = $request-> correoContacto;
            $detalle->correobol = $request-> CorreoBoletinInfo;
            $detalle->reprelegal = $request-> represenLegal;
            $detalle->contacprin = $request-> contactoPrincipal;
            $detalle->cargo = $request-> Cargo;
            ...
            $detalle->TamEmp = $request->TamEmp ;
            $detalle->rama= $request->rama;
            $detalle->acti= $request->acti;
            $detalle->giro= $request->giro;
            $detalle->fechapago= $request->fechapago;
            $detalle->no_excel=$request->no_excel;
            $detalle->curriculum_ciudadano=$request->curriculum_ciudadano;
            $detalle->calle_comercial=$request->calle_comercial;
            $detalle->colonia_comercial=$request->colonia_comercial;
            $detalle->municipio_comercial=$request->municipio_comercial;
            $detalle->cp_comercial=$request->cp_comercial;
            $detalle->facebook=$request->facebook;
            $detalle->instagram=$request->instagram;
            $detalle->twitter=$request->twitter;
            $detalle->linkedink=$request->linkedink;
            $detalle->paginaweb=$request->paginaweb;
            $detalle->colaboradores=$request->colaboradores_uno;
            $detalle->colaboradores_afiliados=$request->colaboradores_afiliados;
            $detalle->nrp=$request->nrp;
            $detalle->nombre_contabilidad=$request->nombre_contabilidad;
            $detalle->correo_contabilidad=$request->correo_contabilidad;
            $detalle->nombre_rh=$request->nombre_rh;
            $detalle->correo_rh=$request->correo_rh;
            $detalle->nombre_capacitacion=$request->nombre_capacitacion;
            $detalle->correo_capacitacion=$request->correo_capacitacion;
            $detalle->nombre_relaciones=$request->nombre_relaciones;
            $detalle->correo_relaciones=$request->correo_relaciones;
            $detalle->nombre_recepcion=$request->nombre_recepcion;
            $detalle->correo_recepcion=$request->correo_recepcion;
            $detalle->nombre_otro=$request->nombre_otro;
            $detalle->correo_otro=$request->correo_otro;
            $detalle->contacto_ocho=$request->contacto_ocho;
            $detalle->correo_ocho=$request->correo_ocho;
            $detalle->telefono_dos=$request->telefono_dos;
            $detalle->telefono_tres=$request->telefono_tres;
            $detalle->inegi=$request->inegi;
            $detalle->clave_inegi=$request->clave_inegi;
            $detalle->clasificacion=$request->clasificacion;
            $detalle->impor_export=$request->impor_export;
            $detalle->esr=$request->esr;
            $detalle->fecha_esr=$request->fecha_esr;
            $detalle->paises=$request->paises;
            $detalle->mision=$request->mision;
            $detalle->vision=$request->vision;
            $detalle->valores=$request->valores;
            $detalle->mes=$request->mes;
            $detalle->promotor=$request->promotor;
            $detalle->periodo=$request->periodo;
    ...
    public function downloadProspectos($role_id, $rol, $id_user, $correos, $nombre, $telefono, $status, $grupo, $etiquetas, $fechaInicio, $fechaFin, $colaboradores, $busqueda, $rama = false, $estatussocio ){
        ...
        $headings = [
            ...
            'Numero de colaboradores afiliados',
            'NRP',
            'Nombre de la persona en el departamento de contabilidad',
            'Correo del departamento de contabilidad',
            'Nombre de la persona en el departamento de recursos humanos',
            'Correo del departamento de recursos humanos',
            'Nombre de la persona en el departamento de ccapacitacion',
            'Correo del departamento de capacitacion',
            'Nombre de la persona en el departamento de relaciones',
            'Correo del departamento de relaciones',
            'Nombre de la persona en el departamento de recepcion',
            'Correo del departamento de recepcion',
            'Nombre de la persona en otro departamento',
            'Correo de otro departamento',
            'Nombre de contacto Ocho',
            'Correo Ocho',
            'Telefono extra 2',
            'Telefono extra 3',
            'Inegi',
            ...
            'Clasificacion',
            'ESR',
            'Fecha de registro ESR',
            'import  y/o  export',
            'Paises en los que se encuentra la empresa',
            'Mision del a empresa',
        ...
        return (new ProspectosReports($headings,$desarrollo,$id_user, $correos, $nombre, $telefono, $status, $fuente, $etiqueta, $fechaInicio, $fechaFin, $colaboradores, $busqueda, $rama, $estatussocio))->download("{$date}_{$desarrollo}_reporte.xlsx");
    }
